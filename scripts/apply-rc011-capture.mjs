...
if (capture.release !== '0.11.0-rc.1' || capture.status !== 'captured') throw new Error('Captura inválida para a RC 0.11.0-rc.1.');
if (args['evidence-url'] && !args['evidence-url'].startsWith('https://')) throw new Error('A URL da evidência deve usar HTTPS.');
...
if (args.kind === 'build') {
  ...
  if (source.sourceCommit && String(source.sourceCommit).toLowerCase() !== String(capture.sourceCommit).toLowerCase()) {
    throw new Error('A captura pertence a um commit diferente do outro artefato já registrado.');
  }
  source.sourceCommit = capture.sourceCommit;
  source.platforms[capture.platform] = {
    status: 'captured', buildId: capture.buildId, buildNumber: capture.buildNumber,
    artifactUrl: capture.artifactUrl, artifactSha256: capture.artifactSha256,
    artifactSizeBytes: capture.artifactSizeBytes, capturedAt: capture.generatedAt,
  ...
  if (capture.action === 'publish') {
    source.publish = { status: 'captured', groupId: capture.groupId, sourceCommit: capture.sourceCommit, publishedAt: capture.generatedAt, evidenceUrl: args['evidence-url'] ?? null };
  } else {
    source.rollback = { status: 'captured', sourceGroupId: capture.sourceGroupId, rollbackGroupId: capture.groupId, validatedAt: capture.generatedAt, evidenceUrl: args['evidence-url'] ?? null };
  }
  source.updatedAt = new Date().toISOString();
