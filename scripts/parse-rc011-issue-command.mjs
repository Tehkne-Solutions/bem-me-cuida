...
const assertValue = (name, value, command) => {
...
export function parseRc011Command(input) {
  ...
  const parsed = { recognized: true, command, privilege: definition.privilege, sourceCommit: '', runId: '', buildId: '', profileId: '', resultStatus: '', installationMode: '', osVersion: '', suiteResults: '', evidenceUrl: '', buildEvidenceUrl: '', homologationEvidenceUrl: '', servicesEvidenceUrl: '', cycleEvidenceUrl: '' };
  definition.args.forEach((name, index) => { const value = values[index]; assertValue(name, value, command); parsed[name] = value; });
  return parsed;
}
