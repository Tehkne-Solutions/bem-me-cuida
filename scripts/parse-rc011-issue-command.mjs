...
const commandDefinitions = {
  ...
  'build-ios': { privilege: 'admin', args: ['sourceCommit', 'cycleEvidenceUrl'] },
  ...
  'discover-ios': { privilege: 'admin', args: ['sourceCommit'] },
  'capture-ios-latest': { privilege: 'admin', args: ['sourceCommit'] },
  ...
  'ios-session-pr': { privilege: 'admin', args: ['sourceCommit', 'runId'] },
  'multiplatform-review': { privilege: 'write', args: ['sourceCommit'] },
  'collect-android': { privilege: 'admin', args: ['sourceCommit', 'buildId'] },
  'collect-ios': { privilege: 'admin', args: ['sourceCommit', 'buildId'] },
...
const assertValue = (name, value) => {
  ...
  if (name === 'profileId' && !PROFILE_PATTERN.test(value)) throw new Error('profileId de aparelho inválido.');
...
export function parseRc011Command(input) {
  ...
  const parsed = {
    recognized: true, command, privilege: definition.privilege,
    sourceCommit: '', runId: '', buildId: '', profileId: '', resultStatus: '', installationMode: '', osVersion: '', suiteResults: '', evidenceUrl: '',
    buildEvidenceUrl: '', homologationEvidenceUrl: '', servicesEvidenceUrl: '', cycleEvidenceUrl: '',
  };
...
const writeGitHubOutput = (path, parsed) => {
  const keys = ['recognized', 'command', 'privilege', 'sourceCommit', 'runId', 'buildId', 'profileId', 'resultStatus', 'installationMode', 'osVersion', 'suiteResults', 'evidenceUrl', 'buildEvidenceUrl', 'homologationEvidenceUrl', 'servicesEvidenceUrl', 'cycleEvidenceUrl'];
  writeFileSync(path, `${keys.map((key) => `${key}=${String(parsed[key] ?? '')}`).join('\n')}\n`, 'utf8');
};
