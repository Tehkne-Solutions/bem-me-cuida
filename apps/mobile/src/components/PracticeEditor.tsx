...
export function PracticeEditor({ initial, saving, submitLabel, onSubmit, onDeactivate }: Props) {
  const [form, setForm] = useState<CreateCarePracticeInput>({
    title: initial?.title ?? '', category: initial?.category ?? 'breathing', description: initial?.description ?? null,
    targetMinutes: initial?.targetMinutes ?? null, timeLocal: initial?.timeLocal ?? null,
    weekdaysMask: initial?.weekdaysMask ?? 127, reminderEnabled: initial?.reminderEnabled ?? false,
  });
  ...
  async function submit() {
    const parsed = createCarePracticeInputSchema.safeParse(form);
    if (!parsed.success) { setError(parsed.error.issues[0]?.message ?? 'Revise os campos.'); return; }
    setError(null); await onSubmit(parsed.data);
  }
  return <View style={styles.wrapper}>
    ...
    <PrimaryButton testID="routine-save" label={submitLabel} loading={saving} onPress={() => void submit()} />
